...
if (isset($_GET['borrow'])) {
    ...
    $stmt->execute([$transactionId, "Borrow", $date->format('Y-m-d'), $_GET['borrow'], "DVD", $_SESSION['id'], $expDate->format('Y-m-d'), 1]);
	}
		...
		header("Refresh:0; url=home-dvd.php");
		exit;
	}
...
<html>
	...
	<body class="loggedin">
		...
		<div class="content">
			<form action="home-dvd.php?select" method="post">
							...
							<tr>
								...
								<?php 
									if ($_SESSION['role'] !== 'Admin') {
										$itemId = $dvd['ItemId'];
										$disabled = "";
										if ($dvd['AmountAvailable'] == 0) {
											$disabled = "disabled";
										}
										echo "<td>";
										echo "<a href=\"home-dvd.php?borrow=$itemId\" onclick=\"return confirm('Continue to borrow?')\" >";
										...
										echo "<a href=\"home-dvd.php?hold=$itemId\" onclick=\"return confirm('Continue to hold?')\" >";
										echo "<input type='button' $disabled style=";
										if($dvd['AmountAvailable'] == 0) {
											echo "\"color:#FCFCFC; background-color: grey;\"";
										} else {
