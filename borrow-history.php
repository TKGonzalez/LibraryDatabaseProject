...
$holdRequestQuery = "
    ...
    SELECT
        ...
        ExpirationTime DueDate,
        'Hold' RequestType,
        ItemID,
        ItemType,
        Active
    FROM Item JOIN HoldRequest USING(ItemId) JOIN User USING(PSID)
";
...
if ($_GET['StartDate']>"0000-00-00" && $_GET['EndDate']>0000-00-00) {
    ...
    ";
    $holdRequestWhere = "
        WHERE PSID = ? AND RequestTime >= ? AND RequestTime <= ?
    ";

    $stmt = $pdo->prepare($transactionQuery . $transactionWhere . $holdRequestQuery . $holdRequestWhere);
    $stmt->execute([ $_SESSION['id'], $_GET['StartDate'], $_GET['EndDate'], $_SESSION['id'], $_GET['StartDate'], $_GET['EndDate'] ]);
...
elseif ($_GET['StartDate']>"0000-00-00") {
    ...
    ";
    $holdRequestWhere = "
        WHERE PSID = ? AND RequestTime >= ?
    ";

    $stmt = $pdo->prepare($transactionQuery . $transactionWhere . $holdRequestQuery . $holdRequestWhere);
    $stmt->execute([ $_SESSION['id'], $_GET['StartDate'], $_SESSION['id'], $_GET['StartDate'] ]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
elseif ($_GET['EndDate']>"0000-00-00") {
    ...
    $stmt = $pdo->prepare($transactionQuery . $transactionWhere . $holdRequestQuery . $holdRequestWhere);
    $stmt->execute([ $_SESSION['id'], $_GET['EndDate'], $_SESSION['id'], $_GET['EndDate'] ]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
else {
    $transactionWhere = "
